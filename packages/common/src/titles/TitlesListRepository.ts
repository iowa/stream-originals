...
export class TitlesListRepository {
  ...
  async getTitles(
    ...
    pageSize: number,
    streamer?: Streamer,
  ): Promise<TitleListDto[]> {
    const conditions = streamer
    ...
    const allTitleIds = await this.db
      ...
      .innerJoin(
        titleRatingsTable,
        eq(titlesTable.id, titleRatingsTable.titleId),
      )
      .where(conditions)
      .orderBy(desc(titleRatingsTable.voteCount))
      .then(results => results.map(t => t.titleId));

    const pageTitleIds = allTitleIds.slice(
      (page - 1) * pageSize,
      page * pageSize,
    );
    ...
    return pageTitleIds
      ...
      .filter(Boolean) as TitleListDto[];
  }
}
